...
public class EvaluationResultDao {
	...
	public List<EvaluationResult> getEvaluationResults(SearchCondition searchCondition) {
		...
		try {
			results = jdbcTemplate.query(query.toString(), params, new BeanPropertyRowMapper<EvaluationResult>(EvaluationResult.class));
	...
	public Evaluation getEvaluationResults(int branchSeq, int evaluationSeq) {
		StringBuffer query = new StringBuffer();
		query.append("SELECT E.*, ");
		query.append("(SELECT B.BRANCH_NAME FROM pia_branches B WHERE B.BRANCH_SEQ = E.BRANCH_SEQ) AS 'BRANCH', ");
		query.append("DATE_FORMAT(E.REGISTER_DATE, '%Y/%m/%d') AS 'FORM_REGISTER_DATE', ");
		...
		try {
			...
			query.append("  FROM pia_evaluation_item_options O ");
			query.append(" WHERE O.IS_DELETED IS FALSE ");
			query.append("   AND O.EVALUATION_ITEM_SEQ = ? ");
	...
	private int insertEvaluationItem(int evaluationSeq, EvaluationItem item, String itemSql) throws Exception {
		...
		params[idx++] = evaluationSeq;
		params[idx++] = item.getEvaluationGroupSeq();
	...
	private void insertEvaluationItemAndOptions(int evaluationSeq, EvaluationItem item, String itemSql, String optionSql) throws Exception {
		int evaluationItemSeq = insertEvaluationItem(evaluationSeq, item, itemSql);
		if (evaluationItemSeq > 0) {
			for (EvaluationItemOption option : item.getOptions()) {
	...
	public boolean changeEvaluation(Evaluation evaluation) throws Exception {
