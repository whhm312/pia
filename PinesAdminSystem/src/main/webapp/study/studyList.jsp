                                        ...
                                        <table class="table table-hover" id="listTable">
                                            <thead>
                                                <tr>
                                                    ...
                                                    <th class="text-center" width="10%">Term of Schedule</th>
                                                    <th class="text-center" width="15%">Writer</th>
                                                    <th class="text-center" width="15%">Date Updated</th>
                                                </tr>
                                            </thead>
                                            <tbody>
                                            </tbody>
        ...
        <script type="text/javascript">
            ...
            function getSearchTerms() {
                	...
                	$.ajax({
                		...
                		contentType : "application/json; charset=UTF-8",
    					success : function(data, textStatus, jqXHR) {
    						if (data.head.status == 200 || data.head.status == 204) {
            ...
            function getListData() {
	            	...
	            	var branchSeq = $("input:radio[name='branch']:checked").val();
	        		var url = "<%=SERVER_URL%>/branches/" + branchSeq + "/campuses/terms/studies?" + getParams();
	            	$.ajax({
	            		headers : getAjaxHeaders("Authorization", $.session.get("x-auth-token")),
	            		url : url,
	            		type : "GET",
            ...
            function getListSuccessFnc(response) {
	        		...
	        		if (response.head.status == 200) {
	        			...
	        			$.each($data, function(index, item) {
	        				...
	        				html = "";
	        				html += "<tr onClick=\"openDetail('" + item.branchSeq + "', '" + item.campusSeq + "', '" + item.termDetailSeq + "', '" + item.studySeq + "');\" style=\"cursor:pointer\">"; 
			...
			function getParams() {
				...
				var paramWriter = encodeURI($("#writer").val());
            ...
            function openDetail(branchSeq, campusSeq, termDetailSeq, studySeq) {
	        		window.location.href = "studyDetail.jsp?branchSeq="+ branchSeq +"&campusSeq=" + campusSeq +"&termDetailSeq=" + termDetailSeq +"&studySeq=" + studySeq;
            }

            function openNew() {
	        		window.location.href = "studyNew.jsp";
            }
